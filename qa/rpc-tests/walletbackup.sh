...
B4ARGS="-datadir=$D4"
$QBITCOIND $QBITCOINDARGS $B4ARGS &
B4PID=$!
...
B3ARGS="-datadir=$D3"
$QBITCOIND $QBITCOINDARGS $B3ARGS &
B3PID=$!
...
function S {
  ...
  if [[ $TXID == "" ]] ; then
      echoerr "node${1}: error sending ${3} qtbc"
...
function StartThree {
  $QBITCOIND $QBITCOINDARGS $B1ARGS &
  B1PID=$!
  $QBITCOIND $QBITCOINDARGS $B2ARGS &
  B2PID=$!
  $QBITCOIND $QBITCOINDARGS $B3ARGS &
  B3PID=$!
}
